...
The TruffleHog action lives in [`trufflehog/`](./trufflehog/action.yaml) and
is referenced as `JetBrains/security-self-service-action/trufflehog@<ref>`.
...
`only-verified: 'true'` (default) restricts results to secrets the verifier
confirmed are currently live. `'false'` also includes matches
